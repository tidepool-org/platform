...
func (s *Standard) ValidateAuthenticationToken(context Context, authenticationToken string) (AuthenticationDetails, error) {
	if context == nil {
...
func (s *Standard) GetUserPermissions(context Context, requestUserID string, targetUserID string) (Permissions, error) {
	if context == nil {
		return nil, errors.New("client", "context is missing")
	}
	if requestUserID == "" {
...
func (s *Standard) sendRequest(context Context, requestMethod string, requestURL string, responseObject interface{}) error {
