...
// 				Expect(err).To(MatchError("request user id is missing"))
// 				Expect(permissions).To(BeNil())
// 				Expect(server.ReceivedRequests()).To(BeEmpty())
// 			})
...
// 				Expect(err).To(MatchError("target user id is missing"))
// 				Expect(permissions).To(BeNil())
// 				Expect(server.ReceivedRequests()).To(BeEmpty())
// 			})
...
// 				BeforeEach(func() {
// 					token = id.New()
// 					context.AuthClientImpl.ServerSessionTokenOutputs = []testAuth.ServerSessionTokenOutput{{Token: token, Error: nil}}
// 				})
