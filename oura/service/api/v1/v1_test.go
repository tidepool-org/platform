...
var _ = Describe("v1", func() {
	It("PartnerPathEvent is expected", func() {
		Expect(ouraServiceApiV1.PartnerPathEvent).To(Equal("/v1/partners/oura/event/:eventType/:dataType"))
	})
					...
					It("returns the expected routes", func() {
						Expect(router.Routes()).To(ConsistOf(
							PointTo(MatchFields(IgnoreExtras, Fields{
								...
								"PathExp":    Equal(ouraServiceApiV1.PartnerPathEvent),
								"Func":       Not(BeNil()),
							})),
							PointTo(MatchFields(IgnoreExtras, Fields{
		...
		It("returns the expected routes", func() {
			Expect(ouraServiceApiV1.Routes()).To(ConsistOf(
				MatchFields(IgnoreExtras, Fields{
					"Method":  Equal("GET"),
					"Path":    Equal(ouraServiceApiV1.PartnerPathEvent),
					"Handler": Not(BeNil()),
				}),
				MatchFields(IgnoreExtras, Fields{
					"Method":  Equal("POST"),
