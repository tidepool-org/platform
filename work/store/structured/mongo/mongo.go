...
func (s *Store) Poll(ctx context.Context, poll *work.Poll) ([]*work.Work, error) {
	...
	pipeline = append(pipeline, bson.M{"$match": bson.M{"$or": bson.A{
		...
		bson.M{"state": "running", "serialId": bson.M{"$exists": true}},
	}}})
	...
	pipeline = append(pipeline, bson.M{"$group": bson.M{"_id": bson.M{"type": "$type", "serialId": "$serialId"}, "documents": bson.M{"$push": "$$ROOT"}}})

	s.testPipeline(ctx, pipeline)

	// Match any without a serial id or any serial id that does not have one in state running
	pipeline = append(pipeline, bson.M{"$match": bson.M{"$or": bson.A{
		bson.M{"_id.serialId": bson.M{"$exists": false}},
		bson.M{"documents.state": bson.M{"$ne": "running"}},
	}}})
	...
	pipeline = append(pipeline, bson.M{"$project": bson.M{"documents": bson.M{"$cond": bson.M{
		"if":   bson.M{"$ifNull": bson.A{"$_id.serialId", false}},
...
func (s *Store) testPipeline(ctx context.Context, pipeline bson.A) {
	lgr := log.LoggerFromContext(ctx).WithField("pipeline", pipeline)

	cursor, err := s.Aggregate(ctx, pipeline)
	lgr = lgr.WithError(err)
	if err != nil {
		lgr.Warn("Unable to aggregate poll work")
		return
	}
	defer cursor.Close(ctx)
	...
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			lgr.Warn("Returned no documents")
			return
		} else {
			lgr.Warn("Unable to test pipeline")
			return
		}
	...
	lgr.WithField("documents", documents).Warnf("Found %d documents", len(documents))
	return
}
...
func (s *Store) Create(ctx context.Context, create *work.Create) (*work.Work, error) {
	...
	// TODO: Prevent similar with deduplication ID within same type
