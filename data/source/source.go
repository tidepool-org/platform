...
//go:generate go tool go.uber.org/mock/mockgen -source=source.go -destination=test/source_mocks.go -package=test -typed
...
type Client interface {
	...
	GetFromProviderSession(ctx context.Context, providerSessionID string) (*Source, error)
}
...
func (c *Create) Validate(validator structure.Validator) {
	validator.String("providerType", &c.ProviderType).OneOf(auth.ProviderTypes()...)
	validator.String("providerName", &c.ProviderName).Using(auth.ProviderNameValidator)
...
func (s *Source) Validate(validator structure.Validator) {
	...
	validator.String("userId", &s.UserID).Using(user.IDValidator)
	validator.String("providerType", &s.ProviderType).OneOf(auth.ProviderTypes()...)
	validator.String("providerName", &s.ProviderName).Using(auth.ProviderNameValidator)
	if providerSessionIDValidator := validator.String("providerSessionId", s.ProviderSessionID); s.State == StateConnected || s.State == StateError {
		providerSessionIDValidator.Exists().Using(auth.ProviderSessionIDValidator)
	} else {
		providerSessionIDValidator.NotExists()
	}
