...
func NewDistance() *physical.Distance {
	...
	datum.Units = pointer.String(test.RandomStringFromArray(physical.DistanceUnits()))
	datum.Value = pointer.Float64(test.RandomFloat64FromRange(physical.DistanceValueRangeForUnits(datum.Units)))
	return datum
}
...
var _ = Describe("Distance", func() {
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("units kilometers; value missing",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("kilometers")
						datum.Value = nil
					},
					testErrors.WithPointerSource(structureValidator.ErrorValueNotExists(), "/value"),
				),
				...
				Entry("units kilometers; value in range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("kilometers")
						datum.Value = pointer.Float64(160.9344)
					},
				),
				Entry("units kilometers; value out of range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("kilometers")
						datum.Value = pointer.Float64(160.9345)
					},
				...
				Entry("units meters; value in range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("meters")
						datum.Value = pointer.Float64(160934.4)
					},
				),
				Entry("units meters; value out of range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("meters")
						datum.Value = pointer.Float64(160934.5)
					},
				...
				Entry("units miles; value missing",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("miles")
						datum.Value = nil
					},
					testErrors.WithPointerSource(structureValidator.ErrorValueNotExists(), "/value"),
				),
				...
				Entry("units yards; value missing",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("yards")
						datum.Value = nil
					},
					testErrors.WithPointerSource(structureValidator.ErrorValueNotExists(), "/value"),
				),
				...
				Entry("units yards; value in range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("yards")
						datum.Value = pointer.Float64(176000.0)
					},
				),
				Entry("units yards; value out of range (upper)",
					func(datum *physical.Distance) {
						datum.Units = pointer.String("yards")
						datum.Value = pointer.Float64(176000.1)
					},
		...
		Context("Normalize", func() {
			DescribeTable("normalizes the datum",
				...
				Entry("does not modify the datum; units feet",
					func(datum *physical.Distance) { datum.Units = pointer.String("feet") },
				),
				Entry("does not modify the datum; units kilometers",
					func(datum *physical.Distance) { datum.Units = pointer.String("kilometers") },
				),
				Entry("does not modify the datum; units meters",
					func(datum *physical.Distance) { datum.Units = pointer.String("meters") },
				),
				Entry("does not modify the datum; units miles",
					func(datum *physical.Distance) { datum.Units = pointer.String("miles") },
				),
				Entry("does not modify the datum; units yards",
					func(datum *physical.Distance) { datum.Units = pointer.String("yards") },
				),
	...
	Context("DistanceValueRangeForUnits", func() {
		...
		It("returns expected range for units invalid", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("invalid"))
			Expect(minimum).To(Equal(-math.MaxFloat64))
			Expect(maximum).To(Equal(math.MaxFloat64))
		})

		It("returns expected range for units feet", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("feet"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(528000.0))
		})

		It("returns expected range for units kilometers", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("kilometers"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(160.9344))
		})

		It("returns expected range for units meters", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("meters"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(160934.4))
		})

		It("returns expected range for units miles", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("miles"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(100.0))
		})

		It("returns expected range for units yards", func() {
			minimum, maximum := physical.DistanceValueRangeForUnits(pointer.String("yards"))
			Expect(minimum).To(Equal(0.0))
