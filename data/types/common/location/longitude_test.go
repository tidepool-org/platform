	...
	Context("Longitude", func() {
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("units empty",
					func(datum *location.Longitude) { datum.Units = pointer.String("") },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotEqualTo("", "degrees"), "/units"),
				),
				Entry("units invalid",
					func(datum *location.Longitude) { datum.Units = pointer.String("invalid") },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotEqualTo("invalid", "degrees"), "/units"),
				),
				Entry("units degrees",
					func(datum *location.Longitude) { datum.Units = pointer.String("degrees") },
				),
				Entry("value missing",
				...
				Entry("value out of range (lower)",
					func(datum *location.Longitude) { datum.Value = pointer.Float64(-180.1) },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotInRange(-180.1, -180.0, 180.0), "/value"),
				),
				Entry("value in range (lower)",
					func(datum *location.Longitude) { datum.Value = pointer.Float64(-180.0) },
				),
