	...
	Context("Latitude", func() {
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("units empty",
					func(datum *location.Latitude) { datum.Units = pointer.String("") },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotEqualTo("", "degrees"), "/units"),
				),
				Entry("units invalid",
					func(datum *location.Latitude) { datum.Units = pointer.String("invalid") },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotEqualTo("invalid", "degrees"), "/units"),
				),
				Entry("units degrees",
					func(datum *location.Latitude) { datum.Units = pointer.String("degrees") },
				),
				Entry("value missing",
				...
				Entry("value out of range (lower)",
					func(datum *location.Latitude) { datum.Value = pointer.Float64(-90.1) },
					testErrors.WithPointerSource(structureValidator.ErrorValueNotInRange(-90.1, -90.0, 90.0), "/value"),
				),
				Entry("value in range (lower)",
					func(datum *location.Latitude) { datum.Value = pointer.Float64(-90.0) },
				),
