...
var _ = Describe("Accuracy", func() {
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("units empty",
					func(datum *location.Accuracy, units *string) {},
					pointer.String(""),
					testErrors.WithPointerSource(structureValidator.ErrorValueStringNotOneOf("", []string{"feet", "meters"}), "/units"),
				),
				Entry("units invalid",
					func(datum *location.Accuracy, units *string) {},
				...
				Entry("units missing; value in range (lower)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(-0.1) },
					nil,
					testErrors.WithPointerSource(structureValidator.ErrorValueNotExists(), "/units"),
				),
				Entry("units missing; value in range (upper)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(3281.9) },
					nil,
					testErrors.WithPointerSource(structureValidator.ErrorValueNotExists(), "/units"),
				),
				...
				Entry("units feet; value in range (lower)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(0.0) },
					pointer.String("feet"),
				),
				Entry("units feet; value in range (upper)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(3280.8) },
					pointer.String("feet"),
				),
				...
				Entry("units meters; value in range (upper)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(1000.0) },
					pointer.String("meters"),
				),
				Entry("units meters; value out of range (upper)",
					func(datum *location.Accuracy, units *string) { datum.Value = pointer.Float64(1000.1) },
					pointer.String("meters"),
	...
	Context("AccuracyValueRangeForUnits", func() {
		...
		It("returns expected range for units invalid", func() {
			minimum, maximum := location.AccuracyValueRangeForUnits(pointer.String("invalid"))
			Expect(minimum).To(Equal(-math.MaxFloat64))
			Expect(maximum).To(Equal(math.MaxFloat64))
		})

		It("returns expected range for units feet", func() {
			minimum, maximum := location.AccuracyValueRangeForUnits(pointer.String("feet"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(1000.0 / 0.3048))
		})

		It("returns expected range for units meters", func() {
			minimum, maximum := location.AccuracyValueRangeForUnits(pointer.String("meters"))
			Expect(minimum).To(Equal(0.0))
