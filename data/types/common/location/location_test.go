		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				func(mutator func(datum *location.Location), expectedErrors ...error) {
					datum := dataTypesCommonLocationTest.NewLocation()
					mutator(datum)
				...
				Entry("gps missing; name missing",
					...
					errorsTest.WithPointerSource(structureValidator.ErrorValueNotExists(), "/gps"),
				),
				Entry("gps invalid",
				...
				Entry("gps valid",
					func(datum *location.Location) { datum.GPS = dataTypesCommonLocationTest.NewGPS() },
				),
				Entry("name missing",
			...
			DescribeTable("normalizes the datum",
				func(mutator func(datum *location.Location)) {
					for _, origin := range structure.Origins() {
						datum := dataTypesCommonLocationTest.NewLocation()
						mutator(datum)
						expectedDatum := dataTypesCommonLocationTest.CloneLocation(datum)
