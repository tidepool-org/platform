		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("acting type invalid",
					func(datum *insulin.Simple) { datum.ActingType = pointer.String("invalid") },
				...
				Entry("acting type intermediate",
					func(datum *insulin.Simple) { datum.ActingType = pointer.String("intermediate") },
				),
				Entry("acting type long",
					func(datum *insulin.Simple) { datum.ActingType = pointer.String("long") },
				),
				Entry("acting type rapid",
					func(datum *insulin.Simple) { datum.ActingType = pointer.String("rapid") },
				),
				Entry("acting type short",
					func(datum *insulin.Simple) { datum.ActingType = pointer.String("short") },
				),
				Entry("brand missing",
					func(datum *insulin.Simple) { datum.Brand = nil },
				),
				Entry("brand empty",
					func(datum *insulin.Simple) { datum.Brand = pointer.String("") },
					testErrors.WithPointerSource(structureValidator.ErrorValueEmpty(), "/brand"),
				),
				Entry("brand invalid",
					func(datum *insulin.Simple) { datum.Brand = pointer.String(test.NewText(101, 101)) },
					testErrors.WithPointerSource(structureValidator.ErrorLengthNotLessThanOrEqualTo(101, 100), "/brand"),
				),
				Entry("brand valid",
					func(datum *insulin.Simple) { datum.Brand = pointer.String(test.NewText(1, 100)) },
				),
