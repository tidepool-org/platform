...
func NewEnergy() *food.Energy {
	...
	datum.Units = pointer.String(test.RandomStringFromArray(food.EnergyUnits()))
	datum.Value = pointer.Float64(test.RandomFloat64FromRange(food.EnergyValueRangeForUnits(datum.Units)))
	return datum
}
...
var _ = Describe("Energy", func() {
	...
	Context("Energy", func() {
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("units invalid",
					func(datum *food.Energy) { datum.Units = pointer.String("invalid") },
	...
	Context("EnergyValueRangeForUnits", func() {
		...
		It("returns expected range for units invalid", func() {
			minimum, maximum := food.EnergyValueRangeForUnits(pointer.String("invalid"))
			Expect(minimum).To(Equal(-math.MaxFloat64))
			Expect(maximum).To(Equal(math.MaxFloat64))
		})

		It("returns expected range for units calories", func() {
			minimum, maximum := food.EnergyValueRangeForUnits(pointer.String("calories"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(10000000.0))
		})

		It("returns expected range for units joules", func() {
			minimum, maximum := food.EnergyValueRangeForUnits(pointer.String("joules"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(41858000.0))
		})

		It("returns expected range for units kilocalories", func() {
			minimum, maximum := food.EnergyValueRangeForUnits(pointer.String("kilocalories"))
			Expect(minimum).To(Equal(0.0))
			Expect(maximum).To(Equal(10000.0))
		})

		It("returns expected range for units kilojoules", func() {
			minimum, maximum := food.EnergyValueRangeForUnits(pointer.String("kilojoules"))
			Expect(minimum).To(Equal(0.0))
