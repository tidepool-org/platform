	...
	Context("expectedExtended", func() {
		DescribeTable("invalid when", testing.ExpectFieldNotValid,
			...
			Entry("is less than extended", NewExpectedExtendedRawObject(), "expectedExtended", 9.6,
				[]*service.Error{testing.ComposeError(validator.ErrorFloatNotInRange(9.6, 9.7, 100.0), "/expectedExtended", NewMeta())},
			),
			Entry("is greater than upper limit", NewExpectedExtendedRawObject(), "expectedExtended", 100.1,
				[]*service.Error{testing.ComposeError(validator.ErrorFloatNotInRange(100.1, 9.7, 100.0), "/expectedExtended", NewMeta())},
			),
