	...
	Context("expectedExtended", func() {
		DescribeTable("invalid when", testing.ExpectFieldNotValid,
			...
			Entry("is less than extended", NewExpectedExtendedRawObject(), "expectedExtended", 9.6,
				[]*service.Error{testing.ComposeError(service.ErrorValueFloatNotInRange(9.6, 9.7, 100.0), "/expectedExtended", NewMeta())},
			),
			Entry("is greater than upper limit", NewExpectedExtendedRawObject(), "expectedExtended", 100.1,
				[]*service.Error{testing.ComposeError(service.ErrorValueFloatNotInRange(100.1, 9.7, 100.0), "/expectedExtended", NewMeta())},
			),
			...
			Entry("is less than lower limit with expected normal", NewExpectedNormalRawObject(), "expectedExtended", -0.1,
				[]*service.Error{testing.ComposeError(service.ErrorValueFloatNotInRange(-0.1, 0.0, 100.0), "/expectedExtended", NewMeta())},
			),
			Entry("is greater than upper limit with expected normal", NewExpectedNormalRawObject(), "expectedExtended", 100.1,
				[]*service.Error{testing.ComposeError(service.ErrorValueFloatNotInRange(100.1, 0.0, 100.0), "/expectedExtended", NewMeta())},
			),
		)
	...
	Context("expectedDuration", func() {
		DescribeTable("invalid when", testing.ExpectFieldNotValid,
			...
			Entry("is less than lower limit with expected normal", NewExpectedNormalRawObject(), "expectedDuration", -1,
				[]*service.Error{testing.ComposeError(service.ErrorValueIntegerNotInRange(-1, 0, 86400000), "/expectedDuration", NewMeta())},
			),
			Entry("is greater than upper limit with expected normal", NewExpectedNormalRawObject(), "expectedDuration", 86400001,
				[]*service.Error{testing.ComposeError(service.ErrorValueIntegerNotInRange(86400001, 0, 86400000), "/expectedDuration", NewMeta())},
			),
		)
