...
func AssignDatasetDataIdentityHashes(datasetData []data.Datum) ([]string, error) {
	...
	for _, datasetDatum := range datasetData {
		...
		if err != nil {
			return nil, errors.Wrap(err, "deduplicator", "unable to gather identity fields for datum")
		}

		hash, err := GenerateIdentityHash(fields)
		if err != nil {
			return nil, errors.Wrap(err, "deduplicator", "unable to generate identity hash for datum")
		}
...
func GenerateIdentityHash(identityFields []string) (string, error) {
	if len(identityFields) == 0 {
		return "", errors.New("deduplicator", "identity fields are missing")
	}

	for _, identityField := range identityFields {
		if identityField == "" {
			return "", errors.New("deduplicator", "identity field is empty")
		}
	}
