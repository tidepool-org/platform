...
// ShiftingSaramaEventsRunner manages multiple sarama consumer groups to execute a
// topic-shifting retry process.
//
...
type ShiftingSaramaEventsRunner struct {
...
func NewShiftingSaramaEventsRunner(config SaramaRunnerConfig, logger log.Logger,
	delays []time.Duration) *ShiftingSaramaEventsRunner {

	return &ShiftingSaramaEventsRunner{
		Config:         config,
		Delays:         delays,
		Logger:         logger,
...
// recipient isn't able to call Close(), thereby opening the potential for a panic when writing
// to a closed channel.
...
func (r *ShiftingSaramaEventsRunner) Run(ctx context.Context) error {
	...
	defer func() {
		r.logger(ctx).Debug("SSER: waiting for consumers")
		wg.Wait()
		r.logger(ctx).Debug("SSER: all consumers returned")
		close(errs)
	}()
	...
	for _, topic := range r.Config.Topics {
		for idx, delay := range r.Delays {
			...
			go func(topic string) {
				...
				r.logger(ctx).WithField("topic", topic).Debug("SSER: consumer go proc returning")
			}(topic)
		}
	}
	...
	select {
	case <-ctx.Done():
		r.logger(ctx).Debug("SSER: context is done")
		return nil
	case err := <-errs:
		r.logger(ctx).WithError(err).Debug("SSER: Run(): error from consumer")
		return err
	}
}

func (r *ShiftingSaramaEventsRunner) producerConfig(idx int, delay time.Duration) *sarama.Config {
...
func (r *ShiftingSaramaEventsRunner) buildConsumer(ctx context.Context, idx int,
...
func (r *ShiftingSaramaEventsRunner) logger(ctx context.Context) log.Logger {
...
// ShiftingConsumer shifts messages that failed to be consumed to another topic.
type ShiftingConsumer struct {
	Consumer  asyncevents.SaramaMessageConsumer
	NextTopic string
...
func (c *ShiftingConsumer) Consume(ctx context.Context, session sarama.ConsumerGroupSession,
	...
	if err := c.Consumer.Consume(ctx, session, msg); err != nil {
		txnErr := c.withTxn(func() error {
			...
			case <-ctx.Done():
				...
				return nil
			case c.Producer.Input() <- c.shiftMessage(msg):
				fields := log.Fields{"from": msg.Topic, "to": c.NextTopic}
				c.Logger.WithFields(fields).Debug("shifted")
				return nil
			}
		})
		if txnErr != nil {
			c.Logger.WithError(txnErr).Info("Unable to complete shifting transaction")
			return err
		}
	}
	return nil
...
func (c *ShiftingConsumer) withTxn(f func() error) (err error) {
...
func (c *ShiftingConsumer) shiftMessage(msg *sarama.ConsumerMessage) *sarama.ProducerMessage {
