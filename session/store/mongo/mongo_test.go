	...
	Context("with a new store", func() {
		...
		Context("NewSessionsSession", func() {
			It("returns a new session if no logger specified", func() {
				mongoSession = mongoStore.NewSessionsSession(nil)
			...
			It("returns a new session if logger specified", func() {
				...
				mongoSession = mongoStore.NewSessionsSession(logger)
				Expect(mongoSession).ToNot(BeNil())
				Expect(mongoSession.Logger()).ToNot(BeNil())
			})
		})
		...
		Context("with a new session", func() {
			BeforeEach(func() {
				mongoSession = mongoStore.NewSessionsSession(null.NewLogger())
				Expect(mongoSession).ToNot(BeNil())
			})
