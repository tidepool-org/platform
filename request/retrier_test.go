...
var _ = Describe("Retrier", func() {
	...
	Context("Retry", func() {
		...
		It("delays with exponential backoff between retries", func() {
			...
			// Delays are 20ms, 40ms, and 80ms (1x, 2x, and 4x the base delay). The upper bound is what the first
			// delay being 2x rather than 1x would produce.
			Expect(elapsed).To(BeNumerically(">=", 140*time.Millisecond))
			Expect(elapsed).To(BeNumerically("<", 280*time.Millisecond))
		})

		It("applies jitter to the delay between retries", func() {
			retrier := request.NewRetrier(1, 100*time.Millisecond, 0.2)
			start := time.Now()
			success, err := retrier.Retry(ctx, func(ctx context.Context) (bool, error) {
				invocations++
				return invocations >= 2, nil
			...
			Expect(success).To(BeTrue())
			Expect(invocations).To(Equal(2))
			// With 20% jitter, the single delay is in the range [80ms, 120ms]. The upper bound is what that delay
			// being 2x rather than 1x the base would produce at its lowest.
			Expect(elapsed).To(BeNumerically(">=", 80*time.Millisecond))
			Expect(elapsed).To(BeNumerically("<", 160*time.Millisecond))
		})
