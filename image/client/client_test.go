		...
		authorizeAssertions := func() {
					...
					parameterAssertions := func() {
						...
						Context("with server response", func() {
							...
							When("the server responds with no result", func() {
								BeforeEach(func() {
									requestHandlers = append(requestHandlers, RespondWithJSONEncoded(http.StatusOK, image.ImageArray{}, responseHeaders))
								})

								It("returns successfully", func() {
									result, err := client.List(ctx, userID, filter, pagination)
									Expect(err).ToNot(HaveOccurred())
									Expect(result).To(Equal(image.ImageArray{}))
								})
							})

							When("the server responds with result", func() {
								var responseResult image.ImageArray

								BeforeEach(func() {
									responseResult = imageTest.RandomImageArray(1, 4)
									requestHandlers = append(requestHandlers, RespondWithJSONEncoded(http.StatusOK, responseResult, responseHeaders))
								})

								It("returns successfully", func() {
									Expect(client.List(ctx, userID, filter, pagination)).To(imageTest.MatchImageArray(responseResult))
								})
							})
						})
					}
					...
					Context("with server response", func() {
						BeforeEach(func() {
							requestHandlers = append(requestHandlers,
								...
								VerifyBody(test.MarshalRequestBody(metadata)),
							)
						})
			...
			Context("with id", func() {
				...
				Context("GetContent", func() {
					...
					BeforeEach(func() {
						urlPath = fmt.Sprintf("/v1/images/%s/content", id)
					})
					...
					mediaTypeAssertions := func() {
						Context("without server response", func() {
							...
							It("returns an error when the media type is invalid", func() {
								mediaType = pointer.FromString("invalid")
								result, err := client.GetContent(ctx, id, mediaType)
								errorsTest.ExpectEqual(err, errors.New("media type is invalid"))
								Expect(result).To(BeNil())
							})
						})
						...
						Context("with server response", func() {
							BeforeEach(func() {
								requestHandlers = append(requestHandlers,
									VerifyRequest(http.MethodGet, urlPath),
									VerifyContentType(""),
									VerifyBody(nil),
								)
							})
							...
							When("the server responds with the result", func() {
								var resultBody []byte
								var resultDigestMD5 string
								var resultMediaType string

								BeforeEach(func() {
									resultBody = imageTest.RandomContentBytes()
									resultDigestMD5 = cryptoTest.RandomBase64EncodedMD5Hash()
									resultMediaType = imageTest.RandomMediaType()
									responseHeaders = http.Header{
										"Digest":       []string{fmt.Sprintf("MD5=%s", resultDigestMD5)},
										"Content-Type": []string{resultMediaType},
									}
									requestHandlers = append(requestHandlers, RespondWith(http.StatusOK, resultBody, responseHeaders))
								})
								...
								It("returns successfully", func() {
									...
									Expect(result.DigestMD5).To(Equal(&resultDigestMD5))
									Expect(result.MediaType).To(Equal(&resultMediaType))
								})
							})
						})
					}
					...
					When("the media type is image/jpeg", func() {
						BeforeEach(func() {
							...
							urlPath = fmt.Sprintf("%s/content.jpeg", urlPath)
						})
					...
					When("the media type is image/png", func() {
						BeforeEach(func() {
							...
							urlPath = fmt.Sprintf("%s/content.png", urlPath)
						})
				...
				Context("GetRenditionContent", func() {
					...
					Context("with server response", func() {
						BeforeEach(func() {
							...
							requestHandlers = append(requestHandlers,
								VerifyRequest(http.MethodGet, fmt.Sprintf("/v1/images/%s/rendition/content/%s", id, renditionString)),
								VerifyContentType(""),
								VerifyBody(nil),
							)
						})
