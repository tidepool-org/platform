		...
		Context("Routes", func() {
			It("returns the expected routes", func() {
				Expect(router.Routes()).To(ConsistOf(
					...
					PointTo(MatchFields(IgnoreExtras, Fields{"HttpMethod": Equal(http.MethodGet), "PathExp": Equal("/v1/images/#id/rendition/content/#renditionAsString")})),
					PointTo(MatchFields(IgnoreExtras, Fields{"HttpMethod": Equal(http.MethodGet), "PathExp": Equal("/v1/images/#id/rendition/content/#renditionAsString/*suffix")})),
		...
		Context("with response and request", func() {
						...
						Context("with client", func() {
							...
							parameterAssertions := func() {
								It("responds with an unauthorized error when the client returns an unauthorized error", func() {
									client.ListOutputs = []imageTest.ListOutput{{Images: nil, Error: request.ErrorUnauthorized()}}
									handlerFunc(res, req)
								...
								It("responds with an internal server error when the client returns an unknown error", func() {
									client.ListOutputs = []imageTest.ListOutput{{Images: nil, Error: errorsTest.RandomError()}}
									handlerFunc(res, req)
								...
								It("responds successfully when the client does not return images", func() {
									client.ListOutputs = []imageTest.ListOutput{{Images: image.Images{}, Error: nil}}
									handlerFunc(res, req)
								...
								It("responds successfully when the client returns images", func() {
									images := imageTest.RandomImages(1, 4)
									client.ListOutputs = []imageTest.ListOutput{{Images: images, Error: nil}}
									handlerFunc(res, req)
									Expect(res.WriteHeaderInputs).To(Equal([]int{http.StatusOK}))
									Expect(res.WriteInputs).To(HaveLen(1))
									Expect(json.Marshal(images)).To(MatchJSON(res.WriteInputs[0]))
								})
							}
				...
				Context("CreateWithMetadata", func() {
					...
					Context("responds with JSON", func() {
						...
						Context("with metadata", func() {
							...
							JustBeforeEach(func() {
								req.Body = ioutil.NopCloser(bytes.NewBuffer(test.MustBytes(test.MarshalRequestBody(metadata))))
							})
			...
			Context("with id", func() {
				...
				Context("Get", func() {
					...
					Context("responds with JSON", func() {
						...
						Context("with client", func() {
							...
							It("responds with not found error when the client does not return a image", func() {
				...
				Context("GetContent", func() {
					...
					contentIntentAssertions := func() {
						...
						suffixAssertions := func() {
							...
							When("responds", func() {
								...
								Context("with client", func() {
									...
									AfterEach(func() {
										Expect(client.GetContentInputs).To(Equal([]imageTest.GetContentInput{{ID: id, ContentIntent: contentIntent}}))
										client.AssertOutputsEmpty()
									})
						...
						When("there is a suffix", func() {
							JustBeforeEach(func() {
								req.URL.Path = fmt.Sprintf("%s/%s", req.URL.Path, imageServiceApiV1Test.RandomPathSuffix())
							})
				...
				Context("GetRenditionContent", func() {
					...
					JustBeforeEach(func() {
						req.URL.Path = fmt.Sprintf("/v1/images/%s/rendition/content/%s", id, renditionAsString)
					})
						...
						When("responds", func() {
							...
							When("the path contains an invalid rendition", func() {
								...
								It("responds with bad request and expected error in body", func() {
									...
									Expect(res.HeaderOutput).To(Equal(&http.Header{"Content-Type": []string{"application/json; charset=utf-8"}}))
									Expect(res.WriteInputs).To(HaveLen(1))
									errorsTest.ExpectErrorJSON(image.ErrorValueRenditionNotParsable(renditionAsString), res.WriteInputs[0])
								})
							})
