...
func (c *Client) GetContent(ctx context.Context, id string, contentIntent *string) (*image.Content, error) {
	ctx = log.ContextWithFields(ctx, log.Fields{"id": id, "contentIntent": contentIntent})
	...
	if !result.HasContent() {
		return nil, nil
	} else if contentIntent != nil && *contentIntent != *result.ContentIntent {
		return nil, nil
	}
...
func (c *Client) GetRenditionContent(ctx context.Context, id string, rendition *image.Rendition) (*image.Content, error) {
	...
	renditionAsString := transform.Rendition.String()
	...
	if !result.HasRendition(transform.Rendition) {
		...
		if err = c.ImageUnstructuredStore().PutRenditionContent(ctx, *result.UserID, *result.ID, *result.ContentID, *renditionsID, renditionAsString, renditionReader, options); err != nil {
			return nil, err
		}
	...
	reader, err := c.ImageUnstructuredStore().GetRenditionContent(ctx, *result.UserID, *result.ID, *result.ContentID, *renditionsID, renditionAsString)
	if err != nil {
		return nil, err
	}
