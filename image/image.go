...
type Client interface {
	List(ctx context.Context, userID string, filter *Filter, pagination *page.Pagination) (Images, error)
	...
	GetContent(ctx context.Context, id string, contentIntent *string) (*Content, error)
...
func (c *ContentAttributes) Parse(parser structure.ObjectParser) {
	...
	c.CreatedTime = parser.Time("createdTime", time.RFC3339)
	c.ModifiedTime = parser.Time("modifiedTime", time.RFC3339)
}
...
func (i *Image) Parse(parser structure.ObjectParser) {
	...
	i.CreatedTime = parser.Time("createdTime", time.RFC3339)
	i.ModifiedTime = parser.Time("modifiedTime", time.RFC3339)
	i.DeletedTime = parser.Time("deletedTime", time.RFC3339)
...
func (i Images) Sanitize(details request.Details) error {
	for _, img := range i {
		if err := img.Sanitize(details); err != nil {
			return err
		}
	}
	return nil
...
func ValidateMediaType(value string) error {
	if err := net.ValidateMediaType(value); err != nil {
		return err
	} else if value != MediaTypeImageJPEG && value != MediaTypeImagePNG {
