...
func RandomRenditionStrings() []string {
	...
	for index := range datum {
		datum[index] = RandomRenditionString()
	}
	return datum
}
...
func RandomImage() *image.Image {
	...
	if *datum.Status == image.StatusAvailable {
		...
		datum.Renditions = pointer.FromStringArray(RandomRenditionStrings())
	}
	datum.CreatedTime = pointer.FromTime(test.RandomTimeFromRange(test.RandomTimeMinimum(), time.Now()).Truncate(time.Second))
	if *datum.Status == image.StatusAvailable {
		datum.ModifiedTime = pointer.FromTime(test.RandomTimeFromRange(*datum.CreatedTime, time.Now()).Truncate(time.Second))
...
func CloneImage(datum *image.Image) *image.Image {
	...
	clone.Metadata = CloneMetadata(datum.Metadata)
...
func NewObjectFromImage(datum *image.Image, objectFormat test.ObjectFormat) map[string]interface{} {
	...
	if datum.Metadata != nil {
		object["metadata"] = NewObjectFromMetadata(datum.Metadata, objectFormat)
	}
...
func RandomImageArray(minimumLength int, maximumLength int) image.ImageArray {
	datum := make(image.ImageArray, test.RandomIntFromRange(minimumLength, maximumLength))
	for index := range datum {
...
func CloneImageArray(datum image.ImageArray) image.ImageArray {
	...
	clone := make(image.ImageArray, len(datum))
	for index := range datum {
