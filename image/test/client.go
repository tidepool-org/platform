...
func (c *Client) List(ctx context.Context, userID string, filter *image.Filter, pagination *page.Pagination) (image.ImageArray, error) {
	...
	if c.ListOutput != nil {
		return c.ListOutput.ImageArray, c.ListOutput.Error
	}
...
func (c *Client) GetContent(ctx context.Context, id string, mediaType *string) (*image.Content, error) {
	...
	c.GetContentInputs = append(c.GetContentInputs, GetContentInput{ID: id, MediaType: mediaType})
	if c.GetContentStub != nil {
		return c.GetContentStub(ctx, id, mediaType)
	}
