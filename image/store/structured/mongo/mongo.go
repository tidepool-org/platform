...
func (s *Session) List(ctx context.Context, userID string, filter *image.Filter, pagination *page.Pagination) (image.Images, error) {
...
func (s *Session) Update(ctx context.Context, id string, condition *request.Condition, update *imageStoreStructured.Update) (*image.Image, error) {
	...
	if update.HasUpdates() {
		...
		if update.ContentID != nil {
			...
			min["contentAttributes.createdTime"] = now.Truncate(time.Second)
			set["contentAttributes.modifiedTime"] = now.Truncate(time.Second)
