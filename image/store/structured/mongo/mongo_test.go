...
type CreatedTimeDescending image.Images
...
func SelectAndSort(images image.Images, selector func(i *image.Image) bool) image.Images {
	var selected image.Images
...
func AsInterfaceArray(images image.Images) []interface{} {
	if images == nil {
		return nil
	}
	array := make([]interface{}, len(images))
		...
		Context("with a new session", func() {
			...
			Context("with user id", func() {
				...
				Context("List", func() {
					...
					Context("with data", func() {
						var allResult image.Images

						BeforeEach(func() {
							allResult = imageTest.RandomImages(8, 8)
				...
				Context("DeleteAll", func() {
					...
					Context("with data", func() {
						var originals image.Images

						BeforeEach(func() {
							originals = imageTest.RandomImages(4, 4)
			...
			Context("Get", func() {
				...
				Context("with data", func() {
					...
					BeforeEach(func() {
						allResult = imageTest.RandomImages(3, 3)
						result = allResult[0]
						...
						Context("with updates", func() {
							Context("with content id, content intent, and content attributes", func() {
								BeforeEach(func() {
									...
									update.RenditionsID = nil
									update.Rendition = nil
								})
							...
							Context("with renditions id and rendition", func() {
								...
								BeforeEach(func() {
									...
									renditionAsString = imageTest.RandomRenditionAsString()
									...
									original.Renditions = pointer.FromStringArray(imageTest.RandomRenditionsAsStrings())
									update.ContentID = nil
									update.ContentIntent = nil
									update.ContentAttributes = nil
									update.RenditionsID = pointer.FromString(renditionsID)
									update.Rendition = pointer.FromString(renditionAsString)
								})
								...
								It("returns updated result when the id exists", func() {
									matchAllFields := MatchAllFields(Fields{
										...
										"Renditions":        PointTo(Equal([]string{renditionAsString})),
										"CreatedTime":       Equal(original.CreatedTime),
										"ModifiedTime":      PointTo(BeTemporally("~", time.Now(), time.Second)),
										"DeletedTime":       Equal(original.DeletedTime),
										"Revision":          PointTo(Equal(*original.Revision + 1)),
							...
							Context("with rendition", func() {
								...
								It("returns updated result when the id exists", func() {
									matchAllFields := MatchAllFields(Fields{
										...
										"Renditions":        PointTo(Equal(append(*original.Renditions, renditionAsString))),
										"CreatedTime":       Equal(original.CreatedTime),
										"ModifiedTime":      PointTo(BeTemporally("~", time.Now(), time.Second)),
										"DeletedTime":       Equal(original.DeletedTime),
										"Revision":          PointTo(Equal(*original.Revision + 1)),
