...
type DexcomClient interface {
	...
	GetDataRange(ctx context.Context, lastSyncTime *time.Time, tokenSource oauth.TokenSource) (*dexcom.DataRangeResponse, error)
...
func (t *TaskRunner) updateProviderSession() error {
	...
	if err != nil {
		return t.rescheduleTaskWithResourceError(errors.Wrap(err, "unable to update provider session"))
	} else if providerSession == nil {
		return t.failTaskWithInvalidStateError(errors.New("provider session is missing"))
	}
	t.providerSession = providerSession
...
func (t *TaskRunner) updateDataSource(update *dataSource.Update) error {
	...
	if err != nil {
		return t.rescheduleTaskWithResourceError(errors.Wrap(err, "unable to update data source"))
...
func (t *TaskRunner) updateDataSet(update *data.DataSetUpdate) error {
	...
	if err != nil {
		return t.rescheduleTaskWithResourceError(errors.Wrap(err, "unable to update data set"))
	} else if dataSet == nil {
...
func (t *TaskRunner) fetchDevices(startTime time.Time, endTime time.Time) (dexcom.Devices, data.Data, error) {
	...
	for _, record := range *response.Records {
		if err := validator.New(t.logger).Validate(record); err != nil {
...
func (t *TaskRunner) createDataSet() (*data.DataSet, error) {
	...
	if err != nil {
		return nil, t.rescheduleTaskWithResourceError(errors.Wrap(err, "unable to create data set"))
	}
...
func (t *TaskRunner) retryTaskWithError(err error) error {
	...
	// Exponential fallback factor
	fallbackFactor := float64(int(1) << (retryCount - 1))

	t.task.AppendError(err)
	t.task.RepeatAvailableAfter(availableAfterDurationWithFallbackFactor(fallbackFactor))
	return err
}
