...
var _ = Describe("Dexcom", func() {
	...
	It("DataKeyResumeDataTime is expected", func() {
		Expect(dexcom.DataKeyResumeDataTime).To(Equal("resumeDataTime"))
	})

	It("DataKeyResumeExpirationTime is expected", func() {
		Expect(dexcom.DataKeyResumeExpirationTime).To(Equal("resumeExpirationTime"))
	})
