...
// 				request.Header.Set(auth.TidepoolSessionTokenHeaderKey, token)
...
// 				request.Header.Del(auth.TidepoolSessionTokenHeaderKey)
...
// 					authClient.ValidateSessionTokenOutputs = []testAuth.ValidateSessionTokenOutput{{Details: authDetails, Error: nil}}
// 				})

// 				It("does nothing if the token is unauthorized", func() {
// 					authClient.ValidateSessionTokenOutputs = []testAuth.ValidateSessionTokenOutput{{Details: nil, Error: client.NewUnauthorizedError()}}
// 					middlewareFunc(response, request)
// 					Expect(response.WriteHeaderInputs).To(Equal([]int{0}))
// 				})
...
// 					authClient.ValidateSessionTokenOutputs = []testAuth.ValidateSessionTokenOutput{{Details: nil, Error: errors.New("test")}}
// 					middlewareFunc(response, request)
...
// 				It("returns successfully and sets auth details", func() {
// 					handlerFunc = func(response rest.ResponseWriter, request *rest.Request) {
// 						Expect(response).ToNot(BeNil())
// 						Expect(request).ToNot(BeNil())
...
// 				It("returns successfully and sets and restores auth details", func() {
