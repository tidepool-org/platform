			...
			if details, err := a.authenticate(req); err != nil {
				...
				if details.HasToken() {
					if reqLgr := service.GetRequestLogger(req); reqLgr != nil {
						...
						service.SetRequestLogger(req, reqLgr.WithField("tokenHash", crypto.HexEncodedMD5Hash([]byte(details.Token()))))
					}
				}

				req.Request = req.WithContext(request.NewContextWithAuthDetails(req.Context(), details))
				if details.HasToken() {
					req.Request = req.WithContext(log.NewContextWithLogger(req.Context(), lgr.WithField("tokenHash", crypto.HexEncodedMD5Hash([]byte(details.Token())))))
				}
			}
