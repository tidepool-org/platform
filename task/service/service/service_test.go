...
// 				Expect(svc.Initialize()).To(MatchError("unable to load task store config; timeout is invalid"))
// 			})

// 			It("returns an error if the timeout is invalid during Validate", func() {
// 				storeConfigReporter.Set("timeout", "0")
// 				Expect(svc.Initialize()).To(MatchError("unable to create task store; config is invalid; timeout is invalid"))
// 			})
