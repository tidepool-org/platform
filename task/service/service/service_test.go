...
var _ = Describe("Service", func() {
	...
	Context("with started server, config reporter, and new service", func() {
		...
		BeforeEach(func() {
			...
			storeConfigReporter = configReporter.WithScopes("task", "store")
		...
		Context("Initialize", func() {
			...
			It("returns an error if the timeout is invalid during Load", func() {
				...
				Expect(svc.Initialize()).To(MatchError("service: unable to load task store config; mongo: timeout is invalid"))
			})

			It("returns an error if the timeout is invalid during Validate", func() {
				storeConfigReporter.Set("timeout", "0")
				Expect(svc.Initialize()).To(MatchError("service: unable to create task store; mongo: config is invalid; mongo: timeout is invalid"))
			})
		...
		Context("with being initialized", func() {
			...
			Context("TaskStore", func() {
				It("returns successfully", func() {
					Expect(svc.TaskStore()).ToNot(BeNil())
				})
			})
