...
const (
	...
	TaskMaximumDuration               = 5 * time.Second
...
func (r *Runner) Run(ctx context.Context, tsk *task.Task) bool {
	...
	if taskDuration := time.Since(now); taskDuration > TaskMaximumDuration {
...
func (r *Runner) setup() error {
	...
	durations := []time.Duration{25 * time.Second} // , 30 * time.Second}
