...
vet: check-environment tmp
	@echo "go vet -all -shadow -shadowstrict"
	@cd $(ROOT_DIRECTORY) && \
		find . -mindepth 1 -maxdepth 1 -not -path "./.*" -not -path "./_*" -not -path "./vendor" -type d -exec go vet -all -shadow -shadowstrict {} \; 2> _tmp/govet.out > _tmp/govet.out && \
