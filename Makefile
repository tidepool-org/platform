...
ifdef PLUGIN
	@cd $(ROOT_DIRECTORY) && \
		...
		echo "Plugin $(PLUGIN) is `go run $(GO_BUILD_FLAGS) $${GOWORK_FLAGS:-} plugin/test/visibility.go`."
endif
...
service-build:
ifdef SERVICE
ifneq ($(filter $(SERVICE:services/%=%),$(SERVICES_TO_BUILD)),)
...
ifdef BUILD_SERVICE
	@cd $(ROOT_DIRECTORY) && \
		DOCKER_REPOSITORY="$(DOCKER_REPOSITORY)`[ ! -e go.work ] || echo '-private'`" && \
		$(TIMING_CMD) $(DOCKER_BUILD_CMD) --tag $${DOCKER_REPOSITORY}:development --target=development --file "$(DOCKER_FILE)" . && \
...
clean: clean-bin clean-cover clean-debug clean-generate clean-test
...
.PHONY: CompileDaemon bindir build build-list build-watch buildable ci ci-build \
    ...
    clean-bin clean-cover clean-debug clean-generate clean-test default docker \
    docker-build docker-push format format-write format-write-changed generate \
    ginkgo go-generate go-mod-download go-mod-tidy goimports gopath-implode imports \
    imports-write imports-write-changed init mockgen phony plugin-visibility \
    ...
    services-build-common test test-ginkgo test-ginkgo-until-failure \
    test-ginkgo-watch test-go tmp vet vet-ignore
