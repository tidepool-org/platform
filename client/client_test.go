...
var _ = Describe("Client", func() {
		...
		Context("with started server and new client", func() {
			...
			Context("RequestStreamWithHTTPClient", func() {
				...
				It("returns error if context is missing", func() {
					reader, err = clnt.RequestStreamWithHTTPClient(context.Context(nil), method, url, mutators, requestBody, inspectors, httpClient)
			...
			Context("RequestDataWithHTTPClient", func() {
				...
				It("returns error if context is missing", func() {
					Expect(clnt.RequestDataWithHTTPClient(context.Context(nil), method, url, mutators, requestBody, responseBody, inspectors, httpClient)).To(MatchError("context is missing"))
					Expect(server.ReceivedRequests()).To(BeEmpty())
				})
	...
	Context("ConstructURL", func() {
		...
		BeforeEach(func() {
			originalAddress = testHttp.NewAddress()
			address = originalAddress
		})

		constructURLAssertions := func() {
			It("returns a valid URL with no paths", func() {
				Expect(client.ConstructURL(address)).To(Equal(originalAddress + "/"))
			})

			It("returns a valid URL with one path", func() {
				path := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
				Expect(client.ConstructURL(address, path)).To(Equal(fmt.Sprintf("%s/%s", originalAddress, path)))
			})
			...
			It("returns a valid URL with multiple paths", func() {
				...
				path2 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
				path3 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
				Expect(client.ConstructURL(address, path1, path2, path3)).To(Equal(fmt.Sprintf("%s/%s/%s/%s", originalAddress, path1, path2, path3)))
			})

			It("returns a valid URL with multiple paths that need to be escaped", func() {
				path1 := test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric) + test.RandomStringFromRangeAndCharset(1, 4, " /;,?") + test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric)
				path2 := test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric) + test.RandomStringFromRangeAndCharset(1, 4, " /;,?") + test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric)
				Expect(client.ConstructURL(address, path1, path2)).To(Equal(fmt.Sprintf("%s/%s/%s", originalAddress, url.PathEscape(path1), url.PathEscape(path2))))
			})

			It("returns a valid URL with multiple paths with surrounding slashes", func() {
				path1 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
				path2 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
				Expect(client.ConstructURL(address, "/"+path1+"/", "/"+path2+"/")).To(Equal(fmt.Sprintf("%s/%s/%s", originalAddress, path1, path2)))
			})
		}
	...
	Context("ConstructPath", func() {
		It("returns a valid URL with no paths", func() {
			Expect(client.ConstructPath()).To(Equal("/"))
		})

		It("returns a valid URL with one path", func() {
			path := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
			Expect(client.ConstructPath(path)).To(Equal(fmt.Sprintf("/%s", path)))
		})
		...
		It("returns a valid URL with multiple paths", func() {
			...
			path2 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
			path3 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
			Expect(client.ConstructPath(path1, path2, path3)).To(Equal(fmt.Sprintf("/%s/%s/%s", path1, path2, path3)))
		})

		It("returns a valid URL with multiple paths that need to be escaped", func() {
			path1 := test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric) + test.RandomStringFromRangeAndCharset(1, 4, " /;,?") + test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric)
			path2 := test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric) + test.RandomStringFromRangeAndCharset(1, 4, " /;,?") + test.RandomStringFromRangeAndCharset(1, 4, test.CharsetAlphaNumeric)
			Expect(client.ConstructPath(path1, path2)).To(Equal(fmt.Sprintf("/%s/%s", url.PathEscape(path1), url.PathEscape(path2))))
		})

		It("returns a valid URL with multiple paths with surrounding slashes", func() {
			path1 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
			path2 := test.RandomStringFromRangeAndCharset(1, 8, test.CharsetAlphaNumeric)
			Expect(client.ConstructPath("/"+path1+"/", "/"+path2+"/")).To(Equal(fmt.Sprintf("/%s/%s", path1, path2)))
		})
	})
})
