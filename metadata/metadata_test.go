...
var _ = Describe("Metadata", func() {
	...
	It("MetadataSizeMaximum is expected", func() {
		Expect(metadata.MetadataSizeMaximum).To(Equal(8 * 1024))
	})
		...
		Context("Validate", func() {
			DescribeTable("validates the datum",
				...
				Entry("size out of range (upper)",
					func(datum *metadata.Metadata) {
						*datum = *metadata.NewMetadata()
						(*datum)["size"] = test.RandomStringFromRangeAndCharset(8182, 8182, test.CharsetAlphaNumeric)
					},
					structureValidator.ErrorSizeNotLessThanOrEqualTo(8193, 8192),
				),
