	...
	Context("Store", func() {
		...
		Context("SetOutdated", func() {
			...
			It("With an existing outdated summary with schema migration reason", func() {
				...
				Expect(userContinuousSummaryWritten.Dates.OutdatedSince).ToNot(BeNil())
				Expect(userContinuousSummaryWritten.Dates.OutdatedSince).To(Equal(outdatedSince))
				Expect(*userContinuousSummaryWritten.Periods).ToNot(HaveLen(0))
				Expect(userContinuousSummaryWritten.Dates.LastData).To(Equal(userContinuousSummary.Dates.LastData))
				Expect(userContinuousSummaryWritten.Dates.FirstData).To(Equal(userContinuousSummary.Dates.FirstData))
				Expect(userContinuousSummaryWritten.Dates.LastUpdatedDate).To(Equal(userContinuousSummary.Dates.LastUpdatedDate))
				Expect(userContinuousSummaryWritten.Dates.LastUploadDate).To(Equal(userContinuousSummary.Dates.LastUploadDate))
