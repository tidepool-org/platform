...
func NewUserDataDeletionHandler(ctx context.Context, authStore store.Store) ev.EventHandler {
...
func (u *userDeletionEventsHandler) HandleDeleteUserEvent(payload ev.DeleteUserEvent) error {
	...
	tokenSsn := u.authStore.NewRestrictedTokenSession()
	defer tokenSsn.Close()
	if err := tokenSsn.DeleteAllRestrictedTokens(u.ctx, payload.UserID); err != nil {
	...
	provSsn := u.authStore.NewProviderSessionSession()
	defer provSsn.Close()
	if err := provSsn.DeleteAllProviderSessions(u.ctx, payload.UserID); err != nil {
		errs = append(errs, err)
